...
- [The original ttyd](https://github.com/tsl0922/ttyd) sets one-time passwords from the command line, so depending on your environment, the password may be visible using the `ps` command. To avoid this, we forked and used [the new ttyd](https://github.com/OpenOnDemandJP/ttyd), which allows you to set one-time passwords via environment variables.
- If the host is Ubuntu 24.04 and has an NVIDIA GPU, the visualization applications do not work due to the difference in the glibc version between the container and the host. Commenting out `_OPTION="--nv"` in `oodjp_apps/utils/config.rb` will make it work.